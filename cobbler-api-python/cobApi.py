...
# server = 'http://10.110.158.2/cobbler_api'
cob_url = 'http://10.0.0.186/cobbler_api'
user = 'cobbler'
password = 'cobbler'
...
class CobblerAPI:
    def __init__(self):
        ...
        self.cobber_server = xmlrpc.client.Server(cob_url)
        self.token = self.cobber_server.login(user, password)

    def cobbler_system_add(self,cobbler_system_info):
        try:
            system_id = self.cobber_server.new_system(self.token)
            self.cobber_server.modify_system(system_id, "name", cobbler_system_info['name'], self.token)
            self.cobber_server.modify_system(system_id, "hostname", cobbler_system_info['hostname'], self.token)
            self.cobber_server.modify_system(system_id, 'modify_interface',
                 {
                     "macaddress-%s"%cobbler_system_info['netCardName']: "%s"%cobbler_system_info['macaddress'],
                     "ipaddress-%s"%cobbler_system_info['netCardName']: "%s"%cobbler_system_info['ipaddress'],
                     "gateway-%s"%cobbler_system_info['netCardName']: "%s"%cobbler_system_info['Gateway'],
                     "subnet-%s"%cobbler_system_info['netCardName']: "%s"%cobbler_system_info['subnet'],
                     "static-%s"%cobbler_system_info['netCardName']: "%s"%cobbler_system_info['static'],
                     # "dnsname-%s"%cobbler_system_info['netCardName']: "%s"%cobbler_system_info['dnsname'],
                 }, self.token)
            self.cobber_server.modify_system(system_id, "profile", cobbler_system_info['profile'], self.token)
            self.cobber_server.save_system(system_id, self.token)
            print('add system %s success'%cobbler_system_info['name'])
        except Exception as e:
            print('add %s system failed \n %s'%(cobbler_system_info['name'],e))

    def cobbler_query_system_list(self):
        system_list = self.cobber_server.find_system()
        print(system_list)
        return system_list

    def cobbler_query_system(self,name):
        new_dict = {}
        # query_info = self.cobber_server.get_repo_config_for_system(name)
        query_info = self.cobber_server.get_system(name)
        # for k,v in query_info.items():
        #     print(k,v)
        # print(type(query_info))
        new_dict['profile'] = query_info['profile']
        new_dict['hostname'] = query_info['hostname']
        new_dict['name'] = query_info['name']
        new_dict['mac_address'] = query_info['interfaces']['eno1']['mac_address']
        new_dict['ip_address'] = query_info['interfaces']['eno1']['ip_address']
        print(new_dict)

        # system_obj = self.cobber_server.cobbler_system(remote_server,token)
        # # system 查询
        # out_all = system_obj.find_system_name()
        # print(out_all)
        # out = system_obj.system_name_info('10.110.158.36')
        # print(out)


        return query_info

    def cobbler_system_remove(self,name):
        try:
            self.cobber_server.remove_system(name,self.token)
        except Exception as e:
            print(e)

    def cobbler_sync(self):
        self.cobber_server.sync(self.token)
...
if __name__ == '__main__':
    ...
    try:
        remote_server = xmlrpc.client.Server(cob_url)
        token = remote_server.login(user, password)

        # print(remote_server.ping())  # cobbler服务器状态监测
        #
        print(remote_server.find_distro())
        print(remote_server.find_system(),'find_system')
        print(remote_server.get_systems(),'get_systems')
        ...
        remote_server.modify_system(system_id, "profile", "centos7-x86_64", token)
